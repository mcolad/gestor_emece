        ...
        <div class='card-header'>
            <div style="float:left">		
                    <div class="coupon-banner">
                         <button class='btn btn-admin btn-sm' onclick="window.location.href='?editar=1&id_apli_zip=<?PHP echo $rowp[$countrow]['id_apli_zip'] ?>&id_apli=<?PHP echo $rowp[$countrow]['id_apli'] ?>&apli_padre=<?PHP echo $_GET['apli_padre'] ?>'">EDITAR</button>
                         ...
                         <a id="coupon-btn" class="btn btn-warning btn-sm" data-icon="i"><span id='code_link_<?PHP echo $rowp[$countrow]['id_apli_zip']; ?>' class="copy-btn" data-clipboard-text="<?PHP echo $dom."/".$abs."_zip/".$rowp[$countrow]['id_apli_zip'].".zip"; ?>">Link</span><span id="alert_link_<?PHP echo $rowp[$countrow]['id_apli_zip']; ?>" style="background-color:#090"><strong>Copy!</strong></span></a>
            ...
            <div style="float:right">
                <div style="float:left; vertical-align:text-top" id='star<?PHP echo $rowp[$countrow]['id_apli_zip'] ?>' class="demo">
                    <?PHP if($rowp[$countrow]['id_apli_zip'] == $zip_destacado ){ $checked = 'checked="checked"'; $flag = '';  } else { $checked = ''; $flag = '1'; }  ?>
                    <input <?PHP echo $checked; ?> title='destacar' type="checkbox" id="acepto" class='chk star' name="acepto" onclick="javascript:recibeid('../../gestion/destacado.php', 'tipo=ZIP&apli_padre=<?PHP echo $_GET['apli_padre'] ?>&id_tipo=<?PHP echo $rowp[$countrow]['id_apli_zip']; ?>&id_apli=<?PHP echo $_GET['id_apli']; ?>&flag=<?PHP echo $flag; ?>', '', 'div_null')" ><label><span></span></label>&nbsp;<div id='div_null'></div>
                </div>
                <div style="float:right" id='estado<?PHP echo $rowp[$countrow]['id_apli_zip'] ?>'>		
