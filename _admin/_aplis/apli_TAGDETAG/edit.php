...
<!--    <button class='btn btn-admin btn-sm modalButton' data-toggle='modal' data-src='<?PHP echo $path; ?>/_admin/_aplis/apli_TAG/index.php?id_apli_tagdetag=<?PHP echo $id; ?>' data-width=100% data-target='#myModal'><i class="fas fa-tags"></i>  Tags</button>
--></div>
...
-->                                    <input name='fecha_nota' id='fecha_nota' value='<?PHP  echo $fecha_nota; ?>' type='hidden' class='form-control'/>
                           <!--         <script>$('#fecha_nota').datetimepicker({ footer: true, modal: true, format: 'yyyy-mm-dd HH:MM' });</script>
                            ...
                            <div class='col-md-12'>
                                    <label>Nombre del tagdetag</label> [<?PHP echo $id ?>]<br />
                            ...
                            <div class='col-md-12'>
                              ...
                              <div class="col-12">
                                   ...
                                   <?PHP if($unico == 1){ $checked = 'checked="checked"'; } else { $checked = ''; } ?>
                                   <input id="switch-info3" name="unico" value="1" <?PHP echo $checked ?>  type="checkbox">
                    ...
                    <div class="col-md-8">
                        <div class="row">
                            <div class='col-md-12 card' style="padding:0px">
                                ...
                                <iframe style="border:0px" width="100%" height="520px" src="<?PHP echo $path; ?>/_admin/_aplis/apli_TAG/index.php?id_apli_tagdetag=<?PHP echo $id; ?>"></iframe>
						  <?PHP 
                                      /*  $result_tag = mysqli_query($cnx, "SELECT * FROM apli_TAG WHERE (id_apli_tagdetag LIKE '%".$id.";%') ORDER BY titulo ASC");
                                        while($row_tag = mysqli_fetch_array($result_tag))
                                        {
                                            ...
                                            ?>
                                            <div class="btn <?PHP echo $btn; ?> btn-sm m-1"><?PHP echo $row_tag['titulo'] ?></div>
                                            <?PHP
                                        }*/
                                ?>
                                </div>
