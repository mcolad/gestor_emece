...
if(!empty($_GET['id_apli_pdf']) AND empty($_GET['confirma']))
	{
		...
		$titulo = $row['titulo'];
		?>
			<form name="formu" action="index.php?<?PHP echo date('hms'); ?>&id_apli_pdf=<?PHP  echo $_GET['id_apli_pdf'] ?>&id_apli=<?PHP  echo $_GET['id_apli'] ?>&apli_padre=<?PHP  echo $_GET['apli_padre'] ?>&play=<?PHP echo $play ?>&confirma=1" method="post">
...
<!--                <div class='col-lg-4'>
	...
	else
	{
		...
		$titulo = '';
		?>
            <form name="formu" action="index.php?<?PHP echo date('hms'); ?>&id_apli_pdf=<?PHP echo $id_apli_pdf; ?>&id_apli=<?PHP  echo $_GET['id_apli'] ?>&apli_padre=<?PHP  echo $_GET['apli_padre'] ?>&play=<?PHP echo $play ?>&confirma=1" method="post" enctype="multipart/form-data">
			<input name="id_apli" type="hidden" value="<?PHP  echo $id_apli; ?>"/>
