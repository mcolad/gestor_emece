...
<?PHP
	...
	$order = "id_apli_img desc";
	$var = "id_apli=".$_GET['id_apli']."&apli_padre=".$_GET['apli_padre'];
	list($rowp) = paginador($select, $from, $where, $order, $var, 2);
?>
<style>
	.star {
	visibility:hidden;
	...
	<div class="card">
        <div class='card-header'>
            <div style="float:left">		
                    <button class='btn btn-admin btn-sm' onclick="window.location.href='index.php?editar=1&id_apli_img=<?PHP echo $rowp[$countrow]['id_apli_img'] ?>&id_apli=<?PHP echo $rowp[$countrow]['id_apli'] ?>&apli_padre=<?PHP echo $_GET['apli_padre'] ?>'">EDITAR</button>
            ...
            <div style="float:right">
				<?PHP if($row_destacado['destacado_img'] == $rowp[$countrow]['id_apli_img']){ $checked = 'checked="checked"'; $flag = ''; } else { $checked = ''; $flag = '1'; }  ?>
                <input <?PHP echo $checked; ?> title='Imagen destacada' type='checkbox' class='chk star' onclick="javascript:recibeid('destacado.php', 'id_apli_img=<?PHP echo $rowp[$countrow]['id_apli_img']; ?>&id_apli=<?PHP echo $rowp[$countrow]['id_apli']; ?>&apli_padre=<?PHP echo $_GET['apli_padre']; ?>&flag=<?PHP echo $flag; ?>', '', 'div_null')" /><div id='div_null'></div>
            </div>
