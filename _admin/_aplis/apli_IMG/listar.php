             ...
             <div class='card-header'>
                 <div style="float:left">		
                    <div class="coupon-banner">
                         <button class='btn btn-admin btn-sm' onclick="window.location.href='?editar=1&id_apli_img=<?PHP echo $rowp[$countrow]['id_apli_img'] ?>&id_apli=<?PHP echo $rowp[$countrow]['id_apli'] ?>&apli_padre=<?PHP echo $_GET['apli_padre'] ?>'">EDITAR</button> 
                         ...
                         <a id="coupon-btn" class="btn btn-warning btn-sm" data-icon="i"><span id='code_link_<?PHP echo $rowp[$countrow]['id_apli_img']; ?>' class="copy-btn" data-clipboard-text="<?PHP echo $dom."/".$abs."_fotos/".$rowp[$countrow]['id_apli_img'].".jpg"; ?>">Link</span><span id="alert_link_<?PHP echo $rowp[$countrow]['id_apli_img']; ?>"><strong>Copy!</strong></span></a>
                    </div> 
                    <script>
                ...
                <div style="float:right">
                    <div style="float:left; vertical-align:text-top" id='star<?PHP echo $rowp[$countrow]['id_apli_img'] ?>' class="demo">
                        <?PHP if($rowp[$countrow]['id_apli_img'] == $img_destacado){ $checked = 'checked="checked"'; $flag = '';  } else { $checked = ''; $flag = '1'; }  ?>
                        <input <?PHP echo $checked; ?> type="checkbox"  title='destacar' id="acepto" class='chk star' name="acepto" onclick="javascript:recibeid('../../gestion/destacado.php', 'tipo=IMG&id_tipo=<?PHP echo $rowp[$countrow]['id_apli_img']; ?>&apli_padre=<?PHP echo $_GET['apli_padre'] ?>&id_apli=<?PHP echo $_GET['id_apli']; ?>&flag=<?PHP echo $flag; ?>', '', 'div_null')" ><label><span></span></label>&nbsp;<div id='div_null'></div>
                    </div>
                    <div style="float:right" id='estado<?PHP echo $rowp[$countrow]['id_apli_img'] ?>'>		
