...
if(!empty($_GET['id_apli_img']) AND empty($_GET['confirma']))
	{
		...
		?>
			...
			<form name="formu" action="index.php?<?PHP echo date('hms'); ?>&id_apli_img=<?PHP  echo $_GET['id_apli_img'] ?>&id_apli=<?PHP  echo $_GET['id_apli'] ?>&apli_padre=<?PHP  echo $_GET['apli_padre'] ?>&play=<?PHP echo $play ?>&confirma=1" method="post">
            ...
            <div class="row">
                ...
                <div class='col-md-12'>
                    ...
                    <button class='btn btn-danger' value="Cancelalr" type="reset" onClick="location.href='?play=<?PHP echo $play ?>&id_apli=<?PHP  echo $_GET['id_apli'] ?>&apli_padre=<?PHP  echo $_GET['apli_padre'] ?>'" />Cancelar</button>
                </div>
            </div> 
	...
	else
	{
		...
		?>
            ...
            <form name="formu" id='formu' action="index.php?<?PHP echo date('hms'); ?>&id_apli_img=<?PHP echo $id_apli_img; ?>&id_apli=<?PHP  echo $_GET['id_apli'] ?>&apli_padre=<?PHP  echo $_GET['apli_padre'] ?>&play=<?PHP echo $play ?>&confirma=1"  method="post" enctype="multipart/form-data">
			<input name="id_apli" type="hidden" value="<?PHP  echo $id_apli; ?>"/>
					   ...
					   <!--<script>$('#fecha').datetimepicker({ footer: true, modal: true, format: 'yyyy-mm-dd HH:MM' });</script>
                        ...
                        <div class='col-md-12'>
                          <div class="custom-file">
                            <input name="archivo[]" type="file" class="custom-file-input" id="custom-file-input" accept=".jpg,.jpeg,.gif,.png"  multiple>
                        ...
                        <div class='col-12 text-center'> 
                            <button class='btn btn-success' value="Guardar" type="submit"/>Guardar</button>
                            <button class='btn btn-danger' value="Cancelar" type="reset" onClick="location.href='?play=<?PHP echo $play ?>&id_apli=<?PHP  echo $_GET['id_apli'] ?>&apli_padre=<?PHP  echo $_GET['apli_padre'] ?>'" />Cancelar</button>
                        </div>
 </p> 
               </div>
            </div>
