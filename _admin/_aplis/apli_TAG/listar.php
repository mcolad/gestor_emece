            ...
            <div class="card">
                <div class='card-header'>
                    <div style="float:left">		
                            <button class='btn btn-admin btn-sm' onclick="window.location.href='index.php?editar=1&play=<?PHP echo $play; ?>&id_apli_tagdetag=<?PHP echo $_GET['id_apli_tagdetag'] ?>&id=<?PHP echo $rowp[$countrow]["id_apli_".strtolower($apli)] ?>'">EDITAR</button>&nbsp;<?PHP echo $rowp[$countrow]["id_apli_".strtolower($apli)] ?>
                    </div>
                ...
                <!--  <div class="card-footer">	
                    <?PHP // tagdetag($apli, $rowp[$countrow]["id_apli_$apli"]); ?>	
                    <?PHP $_GET['apli'] = "$apli"; $id = $rowp[$countrow]["id_apli_".strtolower($apli)]; ?><form name="formu_<?PHP echo $id; ?>" method="post" style="display: inline"><?PHP  $_SESSION['session_tagdetag'.$id] = $rowp[$countrow]['id_apli_tagdetag']; 			include('../tagdetag.php');	?></form>      
               </div>--> 
           </div><br />
			<?PHP	
		$countrow++;	
