...
?>
	...
	<div class="card">
        <div class='card-header '>
                <div class="row">
                    <div class='col-auto'>		
                            <button class='btn btn-success btn-sm' onclick="window.location.href='?editar=1&play=<?PHP echo $play; ?>&order_que=<?PHP echo $order_que ?>&order=<?PHP echo $order ?>&filtro_tag=<?PHP echo $filtro_tag; ?>&id=<?PHP echo $rowp[$countrow]["id_apli_".strtolower($apli)] ?>'">EDITAR</button>
                            <button class='btn btn-admin btn-sm modalButton' data-toggle='modal' data-src='<?PHP echo $path ?>_admin/gestion/ver.php?apli=<?PHP echo $apli ?>&id=<?PHP echo $rowp[$countrow]["id_apli_".strtolower($apli)] ?>' data-width=100% data-target='#myModal'><i class="far fa-eye"></i> VER</button>
                    </div>
                    <div class="col">
        ...
        <div class="card-body ">		
                <div class="row">
					<?PHP
                        $result_img = mysqli_query($cnx, "SELECT * FROM apli_IMG WHERE (id_apli = '".$rowp[$countrow]["id_apli_$apli"]."' AND estado > 2)");
                        $row_img = mysqli_fetch_array($result_img)
                    ?>
                    <div class='col-auto'><?PHP  if(!empty($row_img['id_apli_img'])){echo "<a title='ver imagen' href='".$path."/_fotos/".$row_img['id_apli_img']."_original.jpg' onclick=\"return hs.expand(this)\"><img style='padding:5px' width=65px; src='".$path."/_fotos/".$row_img['id_apli_img']."_300x300.jpg'></a>";} ?> </div>
                    <div class='col'><h5><?PHP echo $rowp[$countrow]['titulo']; ?> </h5></div> 
		        </div>
        </div>
        <div class="card-footer">	 
           <?PHP tag($array_id_apli_tagdetag, "$apli", $rowp[$countrow]["id_apli_".strtolower($apli)], $rowp[$countrow]['id_apli_tag'], 0); ?>	
        </div>
    </div>
    <br />
	<?PHP	
