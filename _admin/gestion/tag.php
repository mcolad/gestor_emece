...
if(!empty($_GET['tag']))
{
	...
	if(!empty($_GET['agrega']))
	{
		...
		mysqli_query($cnx, "UPDATE apli_".$_GET['apli']." SET id_apli_tag = '".$insert_id_apli_tagdetag.";' WHERE id_apli_".strtolower($_GET['apli'])." = '".$_GET['id_apli']."';");
	}
...
if(!empty($_GET['tagdeaplis']))
{
	...
	$row = mysqli_fetch_array($result);
	tagdeaplis($_GET['apli'], $_GET["id_apli_".strtolower($_GET['apli'])]);
}
?>