...
while($row_prov = mysqli_fetch_array($result_prov)) 
{
	...
	?>
		...
		{ echo '"'.$row['localidad'].'": "'.$row['id_gps'].'", '; }
			?>} 
			var <?PHP echo $prov_js ?>Array = $.map(<?PHP echo $prov_js ?>, function (value, key) { return { value:key, data2:value ,data: { category: 'Provincia de <?PHP echo $row_prov['provincia'] ?>' }}; });
	<?PHP 
	if($flag == 1){ $team = $prov_js.'Array'; $flag = 0; }
	else{ $team = $team.'.concat('.$prov_js.'Array)'; }
}
?>
		var teams = <?PHP echo $team ?>;
