...
//	if(!empty($_GET['order'])){ $order = $_GET['order']; }
	...
	if ($cantidad>0)
	{
			...
			if($cantidad > $paso)
			{
                ...
                echo "<div class='col-6-md'><ul class='pagination pagination-sm' style='margin-top:10px'> ";
				...
				<div class='col-6-md'>
					...
					<?PHP
                        ...
                        -->        <div class="modal-body" style="height:650px">
                                <iframe height='600px' width='100%' src="<?PHP echo $path; ?>_admin/gestion/ordenar.php?apli_padre=<?PHP echo str_replace('apli_', '', $from); ?>" frameborder="0"></iframe>
                                </div>
			...
			else
			{
				...
				{
					?>
                        <a class='btn modalButton' data-toggle='modal' data-width=100% data-target='#myModal_ordenar'><?PHP svg('svgadmin/list-ol.svg', '20', '20', 'orange'); ?> Ordenar</a>
                        <!-- MODAL -->
                        <div class="modal fade" id="myModal_ordenar" tabindex="-1" role="dialog"  aria-labelledby="myLargeModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
...
//	if(!empty($play_ajax)){ $play_ajax = $_GET['play_ajax']; } else { $play_ajax = 0; }
	...
	{    
			...
			$query = "SELECT * FROM $from $where ORDER BY $order_que $order LIMIT ".($play_ajax*$paso).", ".$paso.";";
			$result = mysqli_query($cnx, $query);
			while( $row = mysqli_fetch_array($result)){ $rowp[] = $row; }
...
//	if(!empty($play)){ $play = $_GET['play']; } else { $play = 0; }
	...
	if(!empty($_GET['order_que'])){ $order_que = $_GET['order_que']; }
	if(!empty($_GET['order'])){ $order = $_GET['order']; }
//	echo "SELECT * FROM $from $where";
	$result = mysqli_query($cnx, "SELECT * FROM $from $where");
	if(!empty($result) AND mysqli_num_rows($result) > 0)
	{ $cantidad = mysqli_num_rows($result); $count = ceil($cantidad/$paso); }else{ $cantidad = 0; }
	if ($cantidad>0)
	{
			$cantidad = mysqli_num_rows($result);
			$count = ceil($cantidad/$paso);
			...
			$query = "SELECT * FROM $from $where ORDER BY $order_que $order LIMIT ".($play*$paso).", ".$paso.";";
//			echo "SELECT * FROM $from $where ORDER BY $order_que $order LIMIT ".($play*$paso).", ".$paso.";";
			$result = mysqli_query($cnx, $query);
			while( $row = mysqli_fetch_array($result)){ $rowp[] = $row; }
