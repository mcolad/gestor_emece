<?PHP
function tag_apli_v2($apli_rel, $apli, $id_apli)
{
...
?>
	...
	<div id='div_<?PHP echo md5($id_apli) ?>' style='clear:both;'>
		<div style='float:left; margin-right:10px'>
			<form class="form-group" name='formu_<?PHP echo md5($id_apli_apli_rel.$id_apli) ?>' style='display: inline;'>				
				<select class='form-control dropdown-toggle' name='select_<?PHP echo md5($id_apli_apli_rel.$id_apli) ?>' 
				onchange="recibeid('/<?PHP echo $abs ?>_admin/gestion/tag_formu_v2.php', 'accion=agrega&id_apli_apli_rel=<?PHP echo $id_apli_apli_rel ?>&apli=<?PHP echo $apli ?>&id_apli=<?PHP echo $id_apli ?>&id='+formu_<?PHP echo md5($id_apli_apli_rel.$id_apli) ?>.select_<?PHP echo md5($id_apli_apli_rel.$id_apli) ?>.value, '', 'div_<?PHP echo md5($id_apli) ?>')">
	...
	<?PHP
		...
		{
			?>
				<a onclick="javascript:recibeid('/<?PHP echo $abs ?>_admin/gestion/tag_formu_v2.php', 'accion=elimina&id_apli_apli_rel=<?PHP echo $id_apli_apli_rel ?>&apli=<?PHP echo $apli ?>&id_apli=<?PHP echo $id_apli ?>&id_elimina=<?PHP echo $value ?>', '', 'div_<?PHP echo md5($id_apli) ?>')" class="btn btn-success btn-sm elimina" onmouseover="document.bgColor='red'"  style='margin-top:4px; color:white' title='eliminar'><?PHP echo $array_todoslostags[$value]; ?></a>
			<?PHP
		}
	}
	?>
