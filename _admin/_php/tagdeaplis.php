<?PHP
function tagdeaplis($apli, $id_apli_tag, $mostrar = 1)
{
	global $cnx, $path, $abs;
	unset($noactivos);	
		...
		echo "<div id='div_".$id_apli_tag."' style='clear:both;'>";
			...
			if($mostrar == 1)
			{
						...
						echo "<select class='form-control form-control-sm' name='activar' onchange=\"recibeid('/".$abs."_admin/gestion/tag.php', 'tagdeaplis=agrega&apli=".$apli."&id_apli_aplis=".$id_apli_tag."&id_apli_tagdetag='+formu_$id_apli_tag.activar.value, '', 'div_".$id_apli_tag."')\">";
			...
			while($row_tag = mysqli_fetch_array($result_tag))
			{
				...
				{ 
					echo "<div class='form-control form-control-sm tag' style='float:left; margin-right:10px'>".$row_tag['titulo']."&nbsp;";
						if($mostrar == 1)
						{
								echo "<a onclick=\"javascript:recibeid('/".$abs."_admin/gestion/tag.php', 'tagdeaplis=elimina&apli=".$apli."&id_apli_aplis=".$id_apli_tag."&id_apli_tagdetag=".$row_tag['id_apli_tagdetag']."', '', 'div_".$id_apli_tag."')\" class='close' data-dismiss='alert' aria-label='close'>&times;</a>";
						}
					echo "</div>"; 
				}
			}
