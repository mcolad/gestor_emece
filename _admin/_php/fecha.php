...
//		$salida = $salida.$date->format('Y');
	...
	{	
		...
		$salida = $salida." de ".$mes." de ";
		$salida = $salida.$date->format('Y');
		$fecha_2 = substr($fecha, 0, 10);
		$dias = (strtotime(date('Y-m-d'))-strtotime($fecha_2))/86400;

		$dias = abs($dias); 


		if( $dias == 0){ $dias = 'Hoy'; } 
		elseif( $dias == 1) { $dias = 'Hace 1 día'; } 
		else { 
			if($dias < 31)
			{
				$dias = 'Hace '.$dias.' días'; 
