...
//		echo "INSERT IGNORE INTO apli_$apli (id_apli_".$apli.", titulo, id_apli_padre) VALUES ('".$id."', 'Sin completar', '".$id_padre."')";
		mysqli_query($cnx, "INSERT IGNORE INTO apli_$apli (id_apli_".$apli.", titulo, id_apli_padre) VALUES ('".$id."', 'Sin completar', '".$_SESSION['id_beca']."')");
	}
	else
	{ 
		$apli = 'fin'; 
...
{ 
	...
	if (empty($formu_paso))
	{
		$apli = $array_aplis[0];
		$id = $_SESSION['id_beca'];
		mysqli_query($cnx, "INSERT IGNORE INTO apli_$apli (id_apli_".$apli.") VALUES ('".$id."')");
	} 
     ...
     select{ background-color: #EEE !important}
        ...
        <div class="row">
          <div class="col-md-8 ml-auto mr-auto text-center">
            <h5 class="description"><?PHP echo $row_data['bajada'] ?></h5>
          </div>
        </div>
...
<form method="post" action="encuesta.php?formu_paso=<?PHP echo $formu_paso+1 ?>">
		<input name="id" value="<?PHP echo $id ?>" type="hidden" />
		<input name="id_padre" value="<?PHP echo $_SESSION['id_beca'] ?>" type="hidden" />
          <div class="row justify-content-center">
          <div class="col-md-8">
               ...
               while($row_column = mysqli_fetch_array($result_column))
               {
                         ...
                         {	
          				...
          				if($row_column['Comment'] == ''){ $comment = $row_column['Field']; } else { $comment = $row_column['Comment']; }
                              ...
                              {
                              ?>
                                   <div class='col-md-<?PHP echo $col ?>  form-group bmd-form-group'>
                                        <label class="bmd-label-floating"><?PHP echo $comment ?></label>
                              ...
                              elseif($row_column['Type'] == 'date')	
                              {
                                   ...
                                   <div class='col-md-<?PHP echo $col ?> form-group bmd-form-group'>
                                        <label class="bmd-label-floating"><?PHP echo $comment ?></label>
                                        <input style="padding:.55rem !important; padding: " class='form-control' <?PHP echo $required ?> <?PHP echo $disabled; ?> value="2000-01-01" name='<?PHP echo $row_column['Field'] ?>' type='date'/>
                                   </div>  
                                   <?PHP
                              }
                              elseif($row_column['Type'] == 'tinyint(1)')	
                              {
                                   ...
                                   <label> <?PHP echo $comment ?>
							<?PHP if(${$row_column['Field']} == 1){ $checked = 'checked="checked"'; } else { $checked = ''; } ?><br />
                                   <input name='<?PHP echo $row_column['Field'] ?>' type="checkbox" value="1" <?PHP echo $checked ?>>
                              ...
                              ?>
                                   <div class='col-md-<?PHP echo $col ?> form-group bmd-form-group'>
                                        <label class="bmd-label-floating"><?PHP echo $comment ?></label>
                                        <input class='form-control' <?PHP echo $required ?> <?PHP echo $disabled; ?>  name='<?PHP echo $row_column['Field'] ?>' value='<?PHP  echo ${$row_column['Field']}; ?>' type='text'/>
                                   </div> 
                              <?PHP
                              }
                         }
