...
//	$raiz = "";   
	...
	$dom = 'localhost';
	$abs = 'gestor_emece/'; // si hay nombre de carpeta se debe poner / al final
//	$abs = 'ingenieria/'; // si hay nombre de carpeta se debe poner / al final

	$database_name = "gestor_bd";
	...
	//if (!substr_count($_SERVER['SERVER_NAME'], $dom)) { echo "ErRor SERVER_NAME"; die; }
...
if(empty($inicializa))
{
	...
	$result = mysqli_query($cnx, "SELECT * FROM apli_TAGDETAG WHERE (estado > 1)"); 
	while($row = mysqli_fetch_array($result))
	{
		...
		$query2 = "SELECT * FROM apli_TAG WHERE( id_apli_tagdetag LIKE '%".$row['id_apli_tagdetag']."%' AND estado > 1)";
...
//		${'array'.$row['id_apli_tagdetag']} = 
	...
	if((strstr(strstr($_SERVER['SCRIPT_FILENAME'], '_admin'), '/', true) == '_admin'))
	{
		...
		if(isset($row_data['acc_adm'])){ $acc_adm = $row_data['acc_adm']; } else { $acc_adm = 0; }					 
	}
